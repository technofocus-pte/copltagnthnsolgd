...
5.  With same procedure create more input as per given details.
   **Input parameter:**
    Input type: Name <br>
    Date type: Text
...
6.  After **When an agent calls the flow** action, click on **(+)** sign and
...
10. click on the **Add a new row** trigger box and configure the trigger: search and select the **Table name** from Table name section and configure the advanced parameters for the trigger:
...
10. Below **Add a new row** action, click on **(+)** sign and select **Add an
    action**.
...
11. In add an action section, search **Send an email (V2)** in the search bar
    and select **send an email (V2)** from **office 365 outlook
    section**.

12. To Configure **Send an email** trigger enter the below given detail in the
    respected section:
...
13. Now, configure the The **Respond to the agent** action sends data or a confirmation message back to the agent. It lets the agent display a reply to the user after the flow completes.
